...
<section class="bg-background py-20 sm:py-28">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        @if ($stories->isEmpty())
            <p class="mt-12 font-body text-sm text-text-secondary">No stories published yet — check back soon.</p>
        @else
                ...
                @foreach ($stories as $story)
                    <article>
                        <div class="relative aspect-[4/5] overflow-hidden rounded-2xl bg-text-secondary/10">
                            <img
                                src="{{ $story->image_url ?? asset('images/blog_page/all_stories_1.png') }}"
                                alt="{{ $story->title }}"
